...
interface Props {
  property: (typeof SgManualFormSchemaKeys)[number] | 'purpose' | 'amount' | 'term' | 'createdAt' | 'updatedAt' | 'interestRate';
...
export const formatApplicationData = ({ property, value }: Props) => {
  ...
  switch (property) {
    ...
    case 'purpose':
      return (value as string) in loanPurposesLabels ? loanPurposesLabels[value as loanPurposesEnum] : (value as string);
    case 'createdAt':
    case 'updatedAt':
