...
export const ProvinceStep = forwardRef<{ getValue: () => unknown }>((_, ref) => {
  ...
  useEffect(() => {
    if (!visitor) return;
    try {
      const stepData = ApplicationSteps[ApplicationStepsEnum.province].validation(
        visitor[ApplicationStepsEnum.province],
      );
      setValue(stepData);
    } catch (error) {
      // do nothing
