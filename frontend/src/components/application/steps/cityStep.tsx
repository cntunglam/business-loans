...
export const CityStep = forwardRef<{ getValue: () => unknown }>((_, ref) => {
  ...
  useEffect(() => {
    if (!visitor) return;
    try {
      const stepData = ApplicationSteps[ApplicationStepsEnum.city].validation(
        visitor[ApplicationStepsEnum.city],
      );
      setValue(stepData);
    } catch (error) {
      // do nothing
