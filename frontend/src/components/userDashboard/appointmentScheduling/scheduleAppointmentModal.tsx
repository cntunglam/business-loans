...
export const ScheduleAppointmentModal: FC<Props> = ({ loanResponse, onClose }) => {
  ...
  const [selected, setSelected] = useState<Prisma.CompanyStoreGetPayload<{ include: { openingHours: true } }> | undefined>(undefined);
    ...
    <RsModal onClose={onClose} fullscreenOnMobile sx={{ px: { xs: 0, md: 4 }, pb: { xs: 0, sm: 0, md: 0 } }}>
      ...
      <Flex sx={{ position: 'absolute', top: '12px', left: '20px' }}>
        <Typography
          ...
          startDecorator={
            selected && (
              <ChevronLeft onClick={() => setStep('date')} sx={{ mr: 2, cursor: 'pointer', display: { sm: 'none', xs: 'block' } }} />
            )
          }
      ...
      <Flex overflow={'auto'} y grow gap2 sx={{ position: 'relative' }}>
        {!selected ? (
          <Flex gap2 pt={2} pb={{ xs: 0, md: 2 }} px={{ xs: 1 }} fullwidth x xst sx={{ flexDirection: { md: 'row', xs: 'column' } }}>
            {data?.map((store) => (
              <StorePreview
                key={store.id}
                store={store}
          ...
          >
            ...
            >
              <Flex y yc fullwidth gap2 p={3} sx={{ boxShadow: 'md', borderRadius: '6px', display: { xs: 'none', md: 'flex' } }}>
              ...
              >
                ...
                {step === 'date' && !sm ? (
                  <Button disabled={!selectedDate} size="lg" onClick={handleConfirmDate} sx={{ width: 'fit-content', mx: 'auto' }}>
                    Select Time
                  </Button>
                ) : (
                  <Button
