...
export const useGetCompanyStores = (companyId: string) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      axios.get<AwaitedRT<typeof getCompanyStores>>(`/company/${companyId}/store`).then((res) => res.data.data),
  });
};
...
export const useGetCompanyNotificationSettings = () => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      axios.get<AwaitedRT<typeof getCompanyNotificationSettings>>("/company/notification").then((res) => res.data.data),
  });
};
...
export const useGetCompanyLeadSettings = () => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      axios.get<AwaitedRT<typeof getCompanyLeadSettings>>("/company/lead-settings").then((res) => res.data.data),
  });
};
...
export const useGetOfferPreferenceSettings = (params: z.infer<typeof getOfferPreferenceSettingsQuerySchema>) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      axios
        .get<AwaitedRT<typeof getOfferPreferenceSettings>>("/company/preference-settings", { params })
...
export const useUpdateOfferPreferenceSettings = () => {
  ...
  return useMutation({
    mutationFn: (args: z.infer<typeof OfferPreferenceSettingsSchema>) =>
      axios
        .put<AwaitedRT<typeof updateOfferPreferenceSettings>>("/company/preference-settings", args)
        .then((res) => res.data),
  });
};
