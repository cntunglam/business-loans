...
export const useCreateReferralLink = () => {
  ...
  return useMutation({
    mutationFn: (args: { name: string; targetUrl: string }) =>
      axios.post("/affiliate/referral-links", args).then((res) => res.data),
  });
};
...
export const useGetReferralLinks = (name?: string) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      axios
        .get<AwaitedRT<typeof getReferralLinksHandler>>("/affiliate/referral-links", { params: { name } })
...
export const useRecordReferralLinkVisit = () => {
  ...
  return useMutation({
    mutationFn: (referralCode: string) =>
      axios.post(`/affiliate/referral-links/${referralCode}`).then((res) => res.data.data),
  });
};
