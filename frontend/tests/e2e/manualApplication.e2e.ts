...
test("Complete manual application flow", async ({ page }) => {
  ...
  await page.locator(`[data-testid="${TEST_IDS.employmentTypeSelect}_listbox"] >> li[role="option"]`, {
  ...
  const monthlyIncomePercentage = (TEST_DATA.monthlyIncome - TEST_DATA.monthlyIncomeMin) / (TEST_DATA.monthlyIncomeMax - TEST_DATA.monthlyIncomeMin);
  ...
  const laborContractOption = page.locator(getTestId(
    TEST_DATA.hasLaborContract === "Yes" ? TEST_IDS.hasLaborContractYes : TEST_IDS.hasLaborContractNo
  ));
