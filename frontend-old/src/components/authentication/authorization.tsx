...
export const Authorization: FC<Props> = ({ requiredRole, children }) => {
  ...
  useEffect(() => {
    ...
    if (user && user.role === UserRoleEnum.BORROWER && pathname.startsWith('/user') && (!data || data.isWithdrawn || data.isExpired)) {
      return navigate('/apply');
    }
  }, [data, isLoading, isLoadingApplication, navigate, pathname, refetch, requiredRole, searchParams, user, isFetching]);
