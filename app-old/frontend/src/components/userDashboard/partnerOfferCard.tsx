      ...
      {isProceeding && (
        <RsModal onClose={() => setIsProceeding(false)} fullscreenOnMobile={false}>
          ...
          <FlexGrid container spacing={4} sx={{ flexGrow: 1 }}>
            <TitleAndValue title="Apply for up to" value={formatApplicationData({ property: 'amount', value: offer?.amount })} />
            <TitleAndValue title={'Annual Rate'} value={formatApplicationData({ property: 'interestRate', value: offer?.interestRate })} />
