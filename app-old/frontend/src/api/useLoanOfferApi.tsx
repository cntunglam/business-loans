...
export const useCreateLoanOffer = () => {
  ...
  return useMutation({
    mutationFn: (args: z.infer<typeof createLoanOfferSchema>) => axios.post(`/loan-offer`, args).then((res) => res.data)
  });
};
...
export const useRemoveOutcome = () => {
  const axios = useAxios({ successToast: true });
  return useMutation({
    mutationFn: (id: string) => axios.put(`/loan-offer/${id}/remove-outcome`).then((res) => res.data)
  });
};
